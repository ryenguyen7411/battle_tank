...
std::vector<Entity*> Quadtree::Retrieve(Entity* _entity)
{
	...
	else if(IsContain(_entity))
	{
		for(int i = 0; i < m_entityList.size(); i++)
		{
			...
			{
				if(m_entityList[i]->IsTaggedAs(TAG_TANK) || 
					m_entityList[i]->IsTaggedAs(TAG_BULLET) || 
					m_entityList[i]->IsTaggedAs(TAG_BRICK) ||
					m_entityList[i]->IsTaggedAs(TAG_CONCRETE) || 
					m_entityList[i]->IsTaggedAs(TAG_OCEAN) || 
					m_entityList[i]->IsTaggedAs(TAG_ITEM) || 
					m_entityList[i]->IsTaggedAs(TAG_SCREENCOLLIDER))
